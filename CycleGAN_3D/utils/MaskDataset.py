...
def resample_mask(image, transform):
    reference_image = image
...
def rotation3d_mask(image, theta_x, theta_y, theta_z):
   """
   This function rotates an image across each of the x, y, z axes by theta_x, theta_y, and theta_z degrees
   respectively
   :param image: An sitk MRI image
   ...
   euler_transform.SetCenter(image_center)
   euler_transform.SetRotation(theta_x, theta_y, theta_z)
   resampled_image = resample_mask(image, euler_transform)
   return resampled_image
...
class MaskDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path, cross_validation=True, data_split='data_split.csv', nth_fold=0,
                 col=5,
        ...
        with open(data_split) as f:
            for line in f:
                linelist = line.split('\n')[0].split(',')
                if int(linelist[-1]) != nth_fold:
                    self.images_list.append(os.path.join(data_path, 'images', linelist[0]))
                    self.labels_list.append(os.path.join(data_path, 'labels', linelist[1]))
                    self.masks_list.append(os.path.join(data_path, 'masks', linelist[0]))
        ...
        with open(data_split) as f:
            for line in f:
                linelist = line.split('\n')[0].split(',')
                self.condition_dict[linelist[0]] = idx
                self.cond_list.append(float(linelist[col]))
                idx += 1

        # self.cond_list = np.array(self.cond_list)
        # self.cond_list = self.cond_list - np.mean(self.cond_list)
        # self.cond_list = self.cond_list / np.std(self.cond_list)
    ...
    def __getitem__(self, index):
        ...
        condition = self.cond_list[self.condition_dict[data_path.split('/')[-1]]]
        ...
        return torch.from_numpy(image_np), torch.from_numpy(label_np), torch.from_numpy(mask_np), condition  # this is the final output to feed the network
...
class Resample(object):
    ...
    def __call__(self, sample):
        ...
        if check is True:
            ...
            mask = resample_sitk_image(mask, spacing=new_resolution, interpolator=_interpolator_mask)
    ...
    def __call__(self, sample):
        ...
        while not contain_label:
            ...
            if Segmentation is False:
                ...
                threshold.SetInsideValue(1)
                threshold.SetOutsideValue(0)
                _mask = threshold.Execute(label)
                _mask_cropped = roiFilter.Execute(_mask)
                label_crop = roiFilter.Execute(label)
                statFilter = sitk.StatisticsImageFilter()
                statFilter.Execute(_mask_cropped)  # mine for GANs
        ...
        image_crop = roiFilter.Execute(image)
        mask_crop = roiFilter.Execute(mask)
...
class Augmentation(object):
    ...
    def __call__(self, sample):
        ...
        if choice == 3:  # Random rotation
            ...
            mask = rotation3d_mask(mask,theta_x,theta_y, theta_z)
        ...
        if choice == 4:  # BSpline Deformation
            ...
            mask = sitk.Resample(mask, bspline)
            return {'image': image, 'label': label, 'mask': mask}
...
class MaskDataSet_testing(torch.utils.data.Dataset):

    def __init__(self, data_list, label_list, mask_list,
                 which_direction='AtoB',
                 transforms=None,
                 train=False,
                 test=False,):
    ...
    def __getitem__(self, item):
        ...
        data_dict = self.data_list[item]
        label_dict = self.label_list[item]
        mask_dict = self.mask_list[item]
        data_path = data_dict["data"]
        label_path = label_dict["label"]
        mask_path = label_dict["label"]
