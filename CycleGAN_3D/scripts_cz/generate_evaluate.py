...
n_fold = 5
eval_all = True
result = '/scratch/czhu62/yc/mri2pet/CycleGAN_3D/results'
...
for exp_name in exp_names:
    ...
    with open(job_file, 'w') as f:
        ...
        if eval_all:
            lines.append('python /scratch/czhu62/yc/mri2pet/CycleGAN_3D/scripts/evaluate.py --root ${ROOT} --dataset ${DATASET} --exp ${EXP} --epoch ${EPOCH} --map ${MAP} --n_fold ${N_FOLD} --result ${RESULT} --all')
        else:
            lines.append('python /scratch/czhu62/yc/mri2pet/CycleGAN_3D/scripts/evaluate.py --root ${ROOT} --dataset ${DATASET} --exp ${EXP} --epoch ${EPOCH} --map ${MAP} --n_fold ${N_FOLD} --result ${RESULT}')
