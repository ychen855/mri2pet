...
import sys
from utils.MaskDataset import *
import utils.MaskDataset as MaskDataset_testing
...
def prepare_batch(image, mask, ijk_patch_indices):
    image_batches = []
    mask_batches = []
...
def inference_single(model, image_path, mask_path, result_path, resample, resolution, patch_size_x,
              patch_size_y, patch_size_z, stride_inplane, stride_layer, condition, batch_size=1):

    # create transformations to image and labels
    transforms1 = [
        MaskDataset_testing.Resample(resolution, resample)
    ]

    transforms2 = [
        MaskDataset_testing.Padding((patch_size_x, patch_size_y, patch_size_z))
    ]
    ...
    sample = {'image': image, 'label': label_tfm, 'mask': mask}
    ...
    image_tfm, label_tfm, mask_tfm = sample['image'], sample['label'], sample['mask']
    ...
    image_batches, mask_batches = prepare_batch(image_np, mask_np, ijk_patch_indices)
    ...
    for i in tqdm(range(len(image_batches))):
        ...
        mask_batch = torch.from_numpy(mask_batch[np.newaxis, :, :, :])
        mask_batch = Variable(mask_batch.cuda())

        # pred = model(batch)
        model.set_input(image_batch, mask_batch, condition)
        model.test()
...
def inference(model, image_folder, image_list, result_folder, resample, resolution, patch_size_x, patch_size_y, patch_size_z, stride_inplane, stride_layer, condition_dict, batch_size=1):
    ...
    mask_path = os.path.join(image_folder, 'masks')

    for f in image_list:
        inference_single(model, os.path.join(image_path, f), os.path.join(mask_path, f), os.path.join(result_folder, f), resample, resolution, patch_size_x, patch_size_y, patch_size_z, stride_inplane, stride_layer, condition_dict[f], batch_size=1)
...
if __name__ == '__main__':
    ...
    with open(opt.data_split) as f:
        ...
        for line in f:
            ...
            condition_dict[linelist[0]] = float(linelist[col_map[opt.feature]])
            image = linelist[0]
            group = linelist[-1]
            if group == str(opt.fold):
