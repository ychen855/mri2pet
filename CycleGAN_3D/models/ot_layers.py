...
class Self_Attn(nn.Module):
    ...
    def forward(self,x):
        ...
        proj_key =  self.key_conv(x).view(m_batchsize,-1, depth*width*height) # B X C x (*W*H)
...
class SAM(nn.Module):
    def __init__(self, n_feat, kernel_size, bias):
        ...
        self.conv2 = spectral_norm(conv(n_feat, 1, kernel_size, bias=bias))
        self.conv3 = spectral_norm(conv(1, n_feat, kernel_size, bias=bias))
...
class Encoder(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, act, bias, scale_unetfeats, csff):
        ...
        if csff:
            self.csff_enc1 = nn.Conv3d(n_feat,                     n_feat,                     kernel_size=1, bias=bias)
            self.csff_enc2 = nn.Conv3d(n_feat+scale_unetfeats,     n_feat+scale_unetfeats,     kernel_size=1, bias=bias)
            self.csff_enc3 = nn.Conv3d(n_feat+(scale_unetfeats*2), n_feat+(scale_unetfeats*2), kernel_size=1, bias=bias)

            self.csff_dec1 = nn.Conv3d(n_feat,                     n_feat,                     kernel_size=1, bias=bias)
            self.csff_dec2 = nn.Conv3d(n_feat+scale_unetfeats,     n_feat+scale_unetfeats,     kernel_size=1, bias=bias)
            self.csff_dec3 = nn.Conv3d(n_feat+(scale_unetfeats*2), n_feat+(scale_unetfeats*2), kernel_size=1, bias=bias)
...
class _NetG(nn.Module):
    def __init__(self, in_c=1, out_c=1, n_feat=64, scale_unetfeats=32, scale_orsnetfeats=32, num_cab=8, kernel_size=3, reduction=[3,5,7], bias=False):
        super(_NetG, self).__init__()

        act=nn.PReLU()
        self.shallow_feat1 = nn.Sequential(spectral_norm(conv(in_c, n_feat, kernel_size, bias=bias)), CAB(n_feat, kernel_size, 3, bias=bias, act=act))
...
class _NetD(nn.Module):
    def __init__(self):
        ...
        def discriminator_block(in_filters, out_filters, step, normalize=False):
                ...
                if normalize:
                    layers.append(nn.LayerNorm([out_filters, int(256/step), int(256/step), int(256/step)]))
                layers.append(nn.LeakyReLU(0.2, inplace=False))
                return layers
        self.model = nn.Sequential(
            *discriminator_block(1, 32, step=2,normalize=False),
            *discriminator_block(32, 64,step=4),
            *discriminator_block(64, 128,step=8),
