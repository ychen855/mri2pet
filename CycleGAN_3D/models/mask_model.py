...
class MaskModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.condition = input[3]
        # self.image_paths = input['A_paths' if AtoB else 'B_paths']

    def forward(self):
        self.fake_B = self.netG_A(self.real_A, self.condition, fwd=True, mask=self.mask)
        self.rec_A = self.netG_B(self.fake_B, self.condition, fwd=False, mask=self.mask)

        self.fake_A = self.netG_B(self.real_B, self.condition, fwd=False, mask=self.mask)
        self.rec_B = self.netG_A(self.fake_A, self.condition, fwd=True, mask=self.mask)
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            self.idt_A = self.netG_A(self.real_B, self.condition, fwd=True, mask=self.mask)
            self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed.
            self.idt_B = self.netG_B(self.real_A, self.condition, fwd=True, mask=self.mask)
